    ...
    try:
        ...
        for item in items:
            ...
            if item.is_dir():
                ...
                structure[item.name] = {"type": "file", "path": item.relative_to(base_path)}
                file_count += 1
...
def format_structure(structure: dict, base_path: Path, level: int = 0) -> list[str]:
    ...
    for name, item in dirs:
        ...
        level_num = (
            level + 2
        )  # Уровень относительно /Основное (0 = Основное, 1 = папка, 2 = подпапка)
...
def generate_hub(hub_path: Path, folder_path: Path, title: str, description: str):
    ...
    lines = [f"# {title}", "", description, "", "## 📁 Структура", ""]
...
def main(vault_path: str | None = None) -> str:
    ...
    for hub in hubs:
        generate_hub(Path(hub["path"]), Path(hub["folder"]), hub["title"], hub["description"])
